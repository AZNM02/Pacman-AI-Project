...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        if not ghostPositions:
            return Directions.STOP
        # Choose the closest ghost to current Pacman position
        distances = [(self.distancer.getDistance(pacmanPosition, gp), gp) for gp in ghostPositions]
        _, target = min(distances, key=lambda x: x[0])

        # Choose the action that minimizes the maze distance to the target
        bestAction = Directions.STOP
        bestDistance = float('inf')
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            d = self.distancer.getDistance(successor, target)
            if d < bestDistance:
                bestDistance = d
                bestAction = action
        return bestAction
