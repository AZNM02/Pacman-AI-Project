...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [
        (PAC, OBS0), (GHOST0, OBS0),
        (PAC, OBS1), (GHOST1, OBS1),
    ]

    # Domains:

    # Positions can be anywhere on the grid (ignoring walls)
    ...
    # Observations are non-negative noisy Manhattan distances
    max_true_dist = (X_RANGE - 1) + (Y_RANGE - 1)
    max_obs = max_true_dist + MAX_NOISE
    variableDomainsDict[OBS0] = list(range(0, max_obs + 1))
    variableDomainsDict[OBS1] = list(range(0, max_obs + 1))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # 1) Start with all CPTs specialized by the evidence
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # 2) For each hidden variable in order: join on it, then eliminate it
        for var in eliminationOrder:
            currentFactorsList, joined = joinFactorsByVariable(currentFactorsList, var)

            # If eliminating would create an invalid factor (only 1 unconditioned),
            # we simply drop it (result would be identically 1).
            if len(joined.unconditionedVariables()) > 1:
                reduced = eliminate(joined, var)
                currentFactorsList.append(reduced)
            # else: discard `joined` and move on

        # 3) Join remaining factors and normalize to form a proper conditional
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Multiply prior by likelihood for every position, INCLUDING jail
        newBeliefs = DiscreteDistribution()
        for s in self.allPositions:
            likelihood = self.getObservationProb(observation, pacPos, s, jail)
            newBeliefs[s] = self.beliefs[s] * likelihood

        # Normalize to make a proper posterior
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            if oldProb == 0:
                continue
            if oldPos == jail:
                newBeliefs[jail] += oldProb
                continue
            posDist = self.getPositionDistribution(gameState, oldPos)
            for newPos, p in posDist.items():
                newBeliefs[newPos] += oldProb * p
        self.beliefs = newBeliefs
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        if not self.legalPositions:
            return
        n = len(self.legalPositions)
        # Cycle through legal positions with modulo
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Build importance weights over current particles
        weights = DiscreteDistribution()
        for p in self.particles:
            w = self.getObservationProb(observation, pacPos, p, jail)
            weights[p] += w  # accumulate for duplicate particles

        # If every particle received zero weight, reinitialise uniformly
        if weights.total() == 0:
            self.initializeUniformly(gameState)
            return

        # Resample particles from the weight distribution
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            # Keep jail sticky if a particle is already in jail
            if oldPos == jail:
                newParticles.append(jail)
                continue

            # Transition model for the next position given oldPos
